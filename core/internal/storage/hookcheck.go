...
func CheckHook(ctx context.Context, parentDataset string, hookArgv []string) HookCheck {
	...
	if !datasetPattern.MatchString(parentDataset) {
		// SAME FIELD, SAME SCREEN, SAME FACTS as the helper endpoint's refusal — the two buttons sit
		...
		//
		// FACTS, NOT ONE STRING, and that is deliberate rather than sloppy: only this one can end
		// "and quince did not run anything", because only this one runs nothing. The parity that
		// matters is the three facts — the example, the leading slash, and which field the value
		// came from — and `TestCheckHookRefusalTellsThemItIsNotAPath` is what holds them, since a
		// comment asserting parity guarantees none (quince#909 review).
