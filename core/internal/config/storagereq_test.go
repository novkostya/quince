...
func TestCheckStoragesDistinguishesAbsentFromEmpty(t *testing.T) {
	absent := CheckStorages(Default(), nil)
	empty := CheckStorages(withStorages(), nil)
...
func TestCheckStoragesAllowsOneDeclaredStorage(t *testing.T) {
	req := CheckStorages(withStorages(StorageEntry{Name: "local", Path: "/backups", Default: true}), nil)
	if !req.OK() {
...
func TestCheckStoragesReportsRetiredEnvVarButDoesNotRefuseOnIt(t *testing.T) {
	...
	bad := CheckStorages(Default(), []string{"QUINCE_BACKUPS=/srv/backups"})
...
func TestExplainNamesTheKeyAndPrintsTheRemedy(t *testing.T) {
	...
	err := CheckStorages(Default(), nil).Explain(&sb, "/data/config.yml")
	if err == nil {
...
func TestExplainEchoesTheRetiredVarAndSuggestsItsPath(t *testing.T) {
	var sb strings.Builder
	_ = CheckStorages(Default(), []string{"QUINCE_BACKUPS=/srv/backups"}).Explain(&sb, "/data/config.yml")
	out := sb.String()
...
func TestExplainWritesNothingWhenSatisfied(t *testing.T) {
	var sb strings.Builder
	err := CheckStorages(withStorages(StorageEntry{Name: "local", Path: "/backups", Default: true}), nil).
