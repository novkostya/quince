...
func (s *Store) DeviceNotificationsEnabled(udid string) (bool, error) {
	var enabled int
	err := s.db.QueryRow(`SELECT enabled FROM device_notification_prefs WHERE udid = ?`, udid).Scan(&enabled)
...
func (s *Store) SetDeviceNotificationsEnabled(udid string, enabled bool) error {
	v := 0
	if enabled {
		v = 1
	}
	_, err := s.db.Exec(
		`INSERT INTO device_notification_prefs (udid, enabled, updated_at) VALUES (?, ?, ?)
		 ON CONFLICT(udid) DO UPDATE SET enabled = excluded.enabled, updated_at = excluded.updated_at`,
		udid, v, time.Now().UTC().Format(time.RFC3339))
	return err
}
