...
| `local/privacy-patterns.txt` exists | absent is normal for anyone but the Operator; `make privacy-check` then no-ops, so you must sanitize by hand against the rule in `CLAUDE.md` |
