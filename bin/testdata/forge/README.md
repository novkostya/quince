...
| `closed-without-merging-names-itself.json` | a PR that closes WITHOUT merging had no event naming it — the comment said `merged / closed`, the jq only ever did `MERGED` (quince#390) | an **approved** PR auto-closed by `--delete-branch` on its stacked base and nobody saw it for 15 min; a force-push in that window made it unrecoverable, so it needed a new number and a re-review |
...
| `issue58-a-gap-is-named-unreconciled.json` | the **caveat on a catch-up** (quince#58, Operator ruling): a dead watch with an empty queue emits `tick-overdue` and — since this ruling — `event=unreconciled` naming the interval bounds. A state diff compares ENDPOINTS, so a PR that opened and closed inside the gap leaves no trace; the ruling does not require finding it, it requires the tool to stop claiming there was nothing to find. The empty queue is the point: without this line the whole report would be `tick-overdue` plus silence, which is what was read as "nothing was missed" four times |
| `issue58-b-the-gap-is-the-watchers-not-the-last-tick.json` | the precedence, and the case the **sibling above cannot catch** — there `last_tick` and `last_watcher_tick` are equal, so it passes under either. `.last_tick` records ANY tick including the foreground catch-up both skills require before arming, so preferring it names a window seconds wide beside a `tick-overdue` of 1036s. Measured on the first live run of the feature; `from` is the **watcher's** cadence, which is what `due` derives from |
| `issue58-c-a-fresh-arm-derives-the-gap-from-due.json` | the **arm window**, which neither sibling reaches: a fresh arm writes a new `.watch` with no `last_watcher_tick` while a stale `due` survives, so the fallback is the only field left. `due - interval` IS the previous watcher tick — the same value `late=` is derived from — so the two lines cannot disagree. Preferring `.last_tick` here reproduced the original defect in a narrower window, measured twice live before it was believed |
...
| `issue273-a-newly-filed-issue-is-news.json` | an issue **numbered above the highwater** emits `issue-new` and wakes — the channel the gap protocol makes the RULING channel, which until quince#273 had no wake at all. The two scanned numbers **at or below** the mark are the control: a version ignoring the mark would emit five events, not three |
| `issue273-a-cold-start-baselines-and-wakes-nobody.json` | the other direction, and the one that decides whether the class is usable: with **no highwater yet**, the mark is established **silently** and nothing is emitted — otherwise a cold start wakes on every open issue at once and the reader learns to ignore the class |
| `issue348-the-highwater-advances-so-issue-new-converges.json` | the **write-back**, and the only fixture here that could catch quince#348: a `sequence`, because a pair asserts what a tick emits and this defect lived entirely in what a tick LEAVES BEHIND. Two issues are news once, the identical scan one tick later is **silent**, and a genuinely new one is still news after that. Against the pre-fix persist — which restored the previous `open_seen` over the fresh one — the highwater never advances and six of these assertions fail |
| `issue348-b-a-first-observation-establishes-the-highwater.json` | the **other persist site**, which the sibling above does not reach — reverting it alone left 64 fixtures passing (review finding on quince#349). It sits under `_was_observed != true`: a declaration with no completed observation, i.e. the first tick of every cold start. The failure there is worse than a repeating event — with no `open_seen` recorded the baseline is established as **0**, and `select($hw > 0)` then drops every `issue-new` forever, on a watch reporting healthy. Tick 0 is the silent baseline; tick 1 files a new issue that MUST wake |
| `issue306-declaring-an-issue-already-in-the-set-does-not-rebaseline-it.json` | quince#306, the diagnosis half. `issue-first-observation` fires on **absence from the previous observation**, not on being newly declared — `via` labels and never decides. Here #83 is already watched `via=referenced` because the session's own open PR names it, so declaring it changes only the label and the diff runs as it always does, emitting a **seven-hour-old** comment. The age is irrelevant and is what made this look wrong: the tool compares against a stored observation, never against the clock. Break the guard to `select(true)` and this one catches it |
| `issue306-a-genuinely-new-issue-baselines-however-old-its-comments.json` | its control, holding the seven-hour-old comment and the declaration FIXED and varying only prior presence — the single variable quince#306 turned on. It pins the other half of the rule: that a genuinely new issue baselines and emits no comment event, which is the defect quince#306 suspected and could not rule out. **It does not catch the `select(true)` mutation** — for a new issue `$o == null` holds anyway — so neither is redundant and each is load-bearing against a different regression |
| `issue282-a-blocked-pr-the-author-answered.json` | four PRs, **three of which must be silent**. A blocked PR that the author has pushed to (`via=commit`) or commented on (`via=comment`) is owed by the reviewer and fires. The one this design exists for is **403**: a commit dated after the verdict whose `committer` differs from its author is a **rebase**, not an answer — `committedDate` is rewritten by one, and `strict: true` makes rebases the routine answer to every merge. Remove the committer test and 403 fires, claiming a human acted when none did |
| `issue290-review-answered-does-not-repeat.json` | its partner, and the assertion that was MISSING: the **same already-answered PR in both observations**, so the condition holds and the transition never happens — it must be **silent**. Until quince#290 the event fired on every tick, a wake per interval on the seat quince#282 had already measured as over-woken. The `updated` backstop line is still expected, because the PR did move; only the typed event must be once-per-transition |
| `issue290-a-second-block-in-one-interval-still-fires.json` | the third of the set, and it stops the transition test OVER-narrowing. Both observations answered but describing **different blocks** — a verdict and its answer inside one interval — must fire again, keyed on `blocked_at` rather than on a boolean. A missed wake is the expensive direction, and the only two-verdicts-in-one-interval case in this project was **79 s** apart, well inside a 90 s tick. None of the three is redundant: the pre-fix code passed the first, and the null-vs-non-null fix passed the first two |
| `issue202-trunk-goes-red.json` | quince#202's reason for existing: `main` was green and is now red, so `trunk-failed` names the branch, the short oid, what it was, and **which checks are not green** — a red trunk with no named check sends the reader to the Actions tab for what the tick already knew. The PR queue is IDENTICAL in both observations, so no wake here can be explained by queue movement |
| `issue202-trunk-recovers-quietly.json` | the other direction, and the one the ruling was explicit about: **wake on `SUCCESS → FAILURE` only.** A recovery resolves what already woke somebody, so waking again to say it is over turns every red into two interruptions. Still EMITTED — `wake_worthy` drops it, this classifier does not |
| `issue202-trunk-pending-is-not-an-answer.json` | `PENDING` is neither verdict. A run in flight reports it with null conclusions, and reading it as either is wrong half the time — so nothing is emitted and the previous answer is carried, exactly as `mergeability` refuses to report `UNKNOWN`. The expectation is **empty on purpose** |
| `issue202-a-repo-with-no-ci-is-not-pending.json` | the **fourth** state, found by measuring rather than by design: `quince-devlog` returns `statusCheckRollup: null` because it has no CI. That is *no answer EVER*, not *no answer yet* — folding it into `PENDING` waits forever for a verdict that cannot come, which is this issue's own failure mode reintroduced by its fix. Announced once as `state=none` |
| `issue202-a-no-ci-repo-stays-quiet.json` | its control, and what makes the state usable: having said it once, the tick never mentions it again. Half the declared forge set is permanently in this state, so a per-tick line would teach a reader to skim the family that carries a red trunk |
| `issue202-a-red-trunk-wakes-the-loop.json` | the pair fixtures prove the event is EMITTED; only a loop fixture proves it **wakes**. One unchanging PR across every tick, so if this run ends, `trunk-failed` is the only thing that could have ended it |
| `issue202-a-recovered-trunk-does-not-wake.json` | its control. Without it the red one would pass against a build that woke on every trunk transition — the noisy failure this family is one careless line from |
| `issue83-a-post-merge-update-says-nothing-is-pending.json` | `--delete-branch` moves `updatedAt` a second or two after the merge and appears in no activity list, so the backstop says `actor=unattributed` — and `/architect` §6 reads that as **go and look**. Across ten merges in one evening the branch-deletion case fired every time and the named cause, an author ticking a checklist box, fired never. A PR already MERGED at the PREVIOUS observation cannot be awaiting anybody, which is the whole content of that investigation. **PR 702 is the control and is not optional**: an unattributed update on an OPEN PR still means go and look, and without it the fixture would pass against a build that stamped `kind=post-merge` on everything
| `issue83-a-post-merge-comment-still-wakes.json` | constraint 3 of the ruling, and the only thing standing between the suppression and a **lost signal**. `kind=post-merge` is withheld from the wake decision, so the case that must never carry the label is a PR somebody actually commented on after it merged. The third clause protects it — a comment lands in `activity` with a newer timestamp, so the event is attributed and never labelled. PR 704 sits in the SAME observation with nothing newer, so one tick produces both a suppressed event and a woken one: a fixture holding only the surviving case would pass against a build that had stopped labelling anything at all
...
| `issue1189-a-truncated-queue-does-not-wake.json` | the **level-versus-edge** distinction, and the only fixture in this directory whose subject is a CONDITION. A full window whose oldest row is open emits `queue-truncated` on **every** tick — asserted twice here for exactly that reason, because a fixture that saw it once could not tell a standing condition from an edge. Waking on it exits on tick 1 of every arm, so the watch never reaches tick 2 and observes nothing between ticks: measured on both boxes on 2026-08-18, six consecutive arms each dying in under fifteen seconds having reported nothing. The report itself is CORRECT and is still printed; only the exit was wrong. The payload is the real shape — 19 merged rows and one open, which is why `count=1` sits beside `limit=20` |
