...
| **2** | **`replaceLocked` resolves before it validates** (D2a), closing quince#754: the `422`-stricter-than-the-file half, the `name: ""` half, and `retention: null` as a third. **Nothing about marshalling changes.** **This slice takes an architect approval alone** — `docs/contracts.md` is not an owned path (quince#953), so nothing in this rung waits on the Operator and the contingency once planned here, splitting the contracts line into its own tiny PR, has no wait to answer to | G4a, G7, a `curl` reproduction through the running handler, and the existing config suites untouched |
