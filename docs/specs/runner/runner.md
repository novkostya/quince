...
- **G1 (no runner needed)** — `preflight` against a table of environments: each rejection names the
  ...
  (quince#32). **Twenty-four** cases, all synthetic — a stub `claude`, fake token files at 600 and
  644, and throwaway private layers (present, absent, empty, world-readable) — needing no runner and
  never reading the Operator's own layer, so the suite runs on CI and on any contributor's box. They
  assert the **refusals**, since preflight's failure paths are its whole product. Each asserts the
  message as well as the exit code: preflight returns 1 for every refusal, so the code alone cannot
  tell *you set an API key* from *this box holds the wrong identity*, and conflating those is how
  someone fixes the wrong thing. Four further cases assert the suite does not read the **ambient
  environment**: `env FOO=1 cmd` adds to the caller's environment rather than replacing it, so a
  case asserting a variable is *absent* by omitting it asserts a property of the shell that ran the
  suite — which is how this suite once passed on one box and failed on another.
- **G1b (quince#44)** — `preflight` refuses a box that cannot reach the private layer, and the
  refusal **names the fix that exists at the time** and **does not claim to know why** the layer is
  unreachable: a private repository returns 404 for both *does not exist* and *not granted*, so a
  guess sends the next reader to recreate a repo when the fix is a one-click grant. An **empty**
  pattern list is refused too, and named as worse than an absent one — it matches nothing and would
