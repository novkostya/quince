...
| **Approver ≠ author** | Implementer authors. **PRs 3, 4, 5 and 8 are code-owned** — they touch `docs/contracts.md` and `docs/quince.design.md`, two of `CODEOWNERS`' six owned paths — so they need `@novkostya`, and an App verdict cannot satisfy it because an App cannot be a code owner. **This spec and PRs 2, 6, 7 are the architect's to approve**: `/docs/specs/**` is deliberately not owned. |
