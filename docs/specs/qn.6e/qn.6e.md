...
| **Approver ≠ author** | Implementer authors. **PR 8 alone is code-owned** — it touches `docs/quince.design.md`, one of `CODEOWNERS`' five owned paths, so it needs `@novkostya`, and an App verdict cannot satisfy that because an App cannot be a code owner. PRs 3, 4 and 5 touch `docs/contracts.md` only, which is **not** owned (quince#953). **This spec and PRs 2, 6, 7 are the architect's to approve**: `/docs/specs/**` is deliberately not owned. |
