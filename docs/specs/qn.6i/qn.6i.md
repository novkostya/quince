...
| **G7** | 8 | **NO FAKE CLOCK — the spec named one and PR 5 did not build it.** The runner waits on a `time.Timer`, and a fake clock would have meant an injected clock interface for one field. Instead the tests set a **15 ms** interval and wait on a **pass**, not on time, so they are deterministic without a clock: a channel closes when work completes. The `0` half sets the interval to zero and asserts the pass counter does not move across ~8 intervals — the one place a real sleep is used, and it is a *negative* whose failure mode is a false pass rather than a flake. Two further gates the spec did not ask for and the live claim needs: shortening the interval takes effect on the wait **already in progress**, and the schedule can be turned back **on** without a restart |
