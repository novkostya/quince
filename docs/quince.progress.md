...
**One-line state.** ⚑ **`qn.6a` is LANDED (built (da), reviewed+landed (db) 2026-07-24, main `3a7b068`) —
the rung chain is COMPLETE; the frontier is the CODE FREEZE + PROCESS REVAMP, with the app soaking on
staging (Operator: redeploy from `3a7b068` to start the soak).** `qn.6a` = soak-ready UI: mobile-first pass over the existing IA, offline devices
