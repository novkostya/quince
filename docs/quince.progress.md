...
**One-line state.** ⚑ **`qn.6a` is BUILT (CI-proven, 2026-07-24 (da), branch `claude/qn6a-soak-ready-ui`,
awaiting architect review); once it lands, the frontier is the freeze + process revamp (with the app
soaking on staging).** `qn.6a` = soak-ready UI: mobile-first pass over the existing IA, offline devices
