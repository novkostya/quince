...
`3720f84`) — the LAST rung is on main. Remaining before the freeze: the qn.6b LAB LEGS (stories
9/10/11 on the Operator's staging redeploy — story 11 gate-vs-device decides candidate C vs B) and
the story-4 flake fix ((di), in review). Then: CODE FREEZE + PROCESS REVAMP, app soaking.** qn.6b = the patched-from-source
