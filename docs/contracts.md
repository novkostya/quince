...
| `tls.cert_file` · `.key_file` | **paths: restart · contents: already live** | `tlsx.Keeper` re-reads the files on rotation, so a renewal already needs no restart. Changing the *paths*, or turning TLS on or off, still needs a restart — but **no longer because of the socket**: since quince#900 the mux is bound on every start, so the TLS half exists whether or not a certificate does. What is missing is the applier that tells the `Keeper` a path moved. |
